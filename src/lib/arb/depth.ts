...
/**
 * Kalshi orderbooks list resting BIDS on both sides ([price_cents, contracts]).
 * A resting NO bid at p is an offer to sell YES at (100 − p), so:
...
export function summarizeKalshiBook(book: KalshiOrderbook): {
  ...
  const bidDepthUsd = yesBids
    .slice(0, 5)
    .reduce((sum, [price, qty]) => sum + (price / 100) * qty, 0);
  const askDepthUsd = noBids
    .slice(0, 5)
    .reduce((sum, [price, qty]) => sum + ((100 - price) / 100) * qty, 0);
